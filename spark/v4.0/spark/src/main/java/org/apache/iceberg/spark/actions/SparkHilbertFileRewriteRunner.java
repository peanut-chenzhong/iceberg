...
/**
 * Spark file rewrite runner that uses Hilbert curve ordering for data clustering.
 *
 * <p>Hilbert curves provide better locality preservation compared to Z-Order curves, especially for
 * high-dimensional data. This makes them more effective for query optimization when filtering on
 * multiple columns.
 *
 * <p>This implementation is part of the Liquid Clustering feature, which provides adaptive,
 * incremental data layout optimization without requiring predefined partition strategies.
 */
class SparkHilbertFileRewriteRunner extends SparkShufflingFileRewriteRunner {
  ...
  private static final Schema HILBERT_SCHEMA =
      new Schema(Types.NestedField.required(0, HILBERT_COLUMN, Types.BinaryType.get()));
  private static final SortOrder HILBERT_SORT_ORDER =
      SortOrder.builderFor(HILBERT_SCHEMA)
          .sortBy(HILBERT_COLUMN, SortDirection.ASC, NullOrder.NULLS_LAST)
          .build();

  /**
   * Controls the amount of bytes interleaved in the Hilbert algorithm. Default is all bytes being
   * interleaved.
   */
  ...
  /**
   * Overrides the sortSchema method to include columns from HILBERT_SCHEMA.
   *
   * <p>This method generates a new Schema object which consists of columns from the original table
   * schema and HILBERT_SCHEMA.
   */
  @Override
  ...
  private Column hilbertValue(Dataset<Row> df) {
    SparkHilbertUDF hilbertUDF =
        new SparkHilbertUDF(hilbertColNames.size(), varLengthContribution, maxOutputSize);
  ...
  private int varLengthContribution(Map<String, String> options) {
    ...
    Preconditions.checkArgument(
        value > 0,
        "Cannot use less than 1 byte for variable length types with Hilbert, '%s' was set to %s",
  ...
  private int maxOutputSize(Map<String, String> options) {
    ...
    Preconditions.checkArgument(
        value > 0,
        "Cannot have the interleaved Hilbert value use less than 1 byte, '%s' was set to %s",
        MAX_OUTPUT_SIZE,
        value);
    return value;
  }
  ...
  private List<String> validHilbertColNames(
    ...
    Preconditions.checkArgument(
        ...
        "Cannot apply Hilbert clustering when no columns are specified");
    ...
    Preconditions.checkArgument(
        ...
        "Cannot apply Hilbert clustering, all columns provided were identity partition columns and cannot be used");
